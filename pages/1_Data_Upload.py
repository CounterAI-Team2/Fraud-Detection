...
st.caption("Upload SAML-D schema CSV, run feature engineering + model scoring, and store results in session state.")
...
if uploaded is not None:
    ...
    flagged_count = int((feat["risk_tier"].isin(["Critical", "High", "Medium"])).sum())

    st.success("Dataset processed successfully.")
    c1, c2, c3 = st.columns(3)
    c1.metric("Total Transactions", f"{len(feat):,}")
    c2.metric("Flagged (Critical/High/Medium)", f"{flagged_count:,}")
    ...
    log_action(
        ...
        details=(
            f"filename={uploaded.name}; row_count={len(feat)}; flagged_count={flagged_count}; "
            f"tiers={tier_counts}"
        ),
    )
