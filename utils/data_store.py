...
STR_CASE_COLUMNS = [
    ...
    "customer_id",
    "status",
    # --- maker-checker identities + roles ---
    "drafted_by",
    "drafted_role",
    "l1_reviewer",
    "l1_role",
    ...
    "reference_number",
    "grounds",
    # --- subject particulars (regulatory STR fields) ---
    "subject_name",
    "subject_dob",
    "subject_nationality",
    "subject_id_type",
...
def append_archive_record(summary: dict[str, Any]) -> dict[str, Any]:
    ...
    row = {
        ...
        "subject_name": summary.get("subject_name", "") or summary.get("customer_name", ""),
        "reference_number": summary.get("reference_number", ""),
        "archived_at": now,
        "archived_by": summary.get("archived_by", ""),
        "summary_json": serialize_payload(summary),
    }
    # Read-modify-write (rather than raw append) so a pre-existing archive CSV with an
    # older, shorter header is migrated to ARCHIVE_COLUMNS instead of mis-aligning rows.
    existing = read_csv_store(ARCHIVE_PATH, ARCHIVE_COLUMNS)
    combined = pd.concat([existing, pd.DataFrame([row])], ignore_index=True)
    write_csv_store(ARCHIVE_PATH, combined, ARCHIVE_COLUMNS)
    return row
